...
class Msbt:
    ...
    def to_json(self, filename):
        ...
        json.dump(
            output,
            open(filename, "w+", encoding="utf-8"), indent=2, sort_keys=True, ensure_ascii=False, cls=JsonEncoder
        )
        ...
        for i in range(entries):
            ...
            while position < len(string_data):
                ...
                if utf16char != b'\x00\x00':
                    string += utf16char.decode("utf-16")
                else:
    ...
    def _serialize_txt2(self):
        ...
        # each entry is a single 32-bit integer representing an offset from the start of section1 to an area in
        # section2
